...
ROOT <- tryCatch(
  ...
  error = function(e) {
    ...
    f    <- sub("--file=", "", args[grep("--file=", args)][1])
...
df <- read.csv(file.path(ROOT, "data/derived/state_year_predictors.csv")) %>%
...
png(file.path(ROOT, "outputs/figures/sdid_main_spec_trend.png"), width = 900, height = 500, res = 120)
...
png(file.path(ROOT, "outputs/figures/sdid_robust_trend.png"), width = 900, height = 500, res = 120)
...
png(file.path(ROOT, "outputs/figures/sdid_main_spec_weights.png"), width = 700, height = 500, res = 120)
synthdid_units_plot(tau_sdid, negligible.threshold = 0.001)
dev.off()
cat("Saved: sdid_main_spec_weights.png\n")

png(file.path(ROOT, "outputs/figures/sdid_robust_weights.png"), width = 700, height = 500, res = 120)
synthdid_units_plot(tau_sdid_r, negligible.threshold = 0.001)
dev.off()
cat("Saved: sdid_robust_weights.png\n")

# ── Block 8: Save CSVs ────────────────────────────────────────────────────────
write.csv(results,          file.path(ROOT, "outputs/tables/sdid_comparison_table.csv"),     row.names = FALSE)
write.csv(w_main$omega,     file.path(ROOT, "outputs/tables/sdid_main_spec_omega.csv"),      row.names = FALSE)
write.csv(w_main$lambda,    file.path(ROOT, "outputs/tables/sdid_main_spec_lambda.csv"),     row.names = FALSE)
write.csv(w_robust$omega,   file.path(ROOT, "outputs/tables/sdid_robust_omega.csv"),         row.names = FALSE)
write.csv(w_robust$lambda,  file.path(ROOT, "outputs/tables/sdid_robust_lambda.csv"),        row.names = FALSE)
